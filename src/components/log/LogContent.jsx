...
const LogContent = ({ logs, logContainerRef, loading, getIcon }) => {
  ...
  return (
    <div
      ref={logContainerRef}
      className="log-entries log-scrollbar flex-1 overflow-y-auto p-4 space-y-2 min-h-0 border-l border-r border-b border-[#cec8bc] dark:border-[#222222] rounded-lg rounded-t-none"
    >
      {logs.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center text-[#666660] dark:text-gray-500">
          <div className="w-12 h-12 mb-2 my-1">
            <img src={log} alt="Logs" className="w-12 h-12 pointer-events-none select-none" />
          </div>
          <p className="text-sm font-medium text-[#666660] dark:text-gray-500">
            {loading ? "Initializing..." : "No activity detected"}
          </p>
          {!loading && (
            <p className="text-xs text-[#666660] dark:text-gray-500 mt-1">
        ...
        segments.map((segment, idx) => {
          ...
          return (
            <div
              key={`group-${idx}`}
              className="border-l-2 border-[#bab4a8] bg-[rgba(26,26,26,0.03)] dark:border-zinc-700 dark:bg-white/2 rounded-r-md pl-3 py-2 flex flex-col gap-2"
            >
              <span className="text-[10px] uppercase tracking-[0.15em] text-[#666660] dark:text-zinc-600 font-semibold">
                {label}
              </span>
